...
# The environment variable that signifies tests are being run with the custom
# TestResult class.
NW_TESTING_VAR = 'NW_UNITTEST'
...
def maya_module_tests():
    """Generator function to iterate over all the Maya module tests
    directories.
    """
...
def run_tests_from_commandline(directories=None, test=None):
    ...
    # When a maya module is loaded, the scripts folder is added to PYTHONPATH,
    # but it doesn't seem to be added to sys.path. So we are unable to import
    # any of the python files that are in the
    # module/scripts folder. To workaround this, we simply add the paths to
    # sys ourselves.
...
class Settings(object):
    ...
    # Use a uuid subdirectory so tests that are running concurrently such as
    # on a build server
    # do not conflict with each other.
    temp_dir = os.path.join(tempfile.gettempdir(), 'mayaunittest',
                            str(uuid.uuid4()))

    # Controls whether temp files should be deleted after running all tests
    # in the test case
    delete_files = True

    # Specifies whether the standard output and standard error streams are
    # buffered during the test run.
    # Output during a passing test is discarded. Output is echoed normally on
    # test fail or error and is
...
def set_delete_files(value):
    """Set whether temp files should be deleted after running all tests in a
    test case.
...
def set_buffer_output(value):
    """Set whether the standard output and standard error streams are buffered
    during the test run.
...
class TestCase(unittest.TestCase):
    ...
    # Keep track of all temporary files that were created so they can be
    # cleaned up after all tests have been run
    files_created = []
    ...
    def load_plugin(cls, plugin):
        """Load the given plug-in and saves it to be unloaded when the TestCase
        is finished.
    ...
    def assertListAlmostEqual(self, first, second, places=7, msg=None,
                              delta=None):
    ...
    def tearDown(self):
        if Settings.file_new and NW_TESTING_VAR not in os.environ.keys():
            # If running tests without the custom runner, like with PyCharm,
            # the file new of the TestResult class isn't used so call file new
            # here
...
class TestResult(unittest.TextTestResult):
    ...
    def startTestRun(self):
        ...
        # Create an environment variable that specifies tests are being run
        # through the custom runner.
        os.environ[NW_TESTING_VAR] = '1'

        ScriptEditorState.suppress_output()
        if Settings.buffer_output:
            # Disable any logging while running tests. By disabling critical,
            # we are disabling logging at all levels below critical as well
...
class ScriptEditorState(object):
    ...
    def suppress_output(cls):
        ...
        if Settings.buffer_output:
            cls.suppress_results = cmds.scriptEditorInfo(q=True,
                                                         suppressResults=True)
            cls.suppress_errors = cmds.scriptEditorInfo(q=True,
                                                        suppressErrors=True)
            cls.suppress_warnings = cmds.scriptEditorInfo(q=True,
                                                          suppressWarnings=True)
            cls.suppress_info = cmds.scriptEditorInfo(q=True,
                                                      suppressInfo=True)
    ...
    def restore_output(cls):
        """Restores the script editor output settings to their original
        values.
        """
        if None not in {cls.suppress_results, cls.suppress_errors,
                        cls.suppress_warnings, cls.suppress_info}:
            cmds.scriptEditorInfo(e=True,
...
if __name__ == '__main__':
    ...
    test = sys.argv[2]
    run_tests_from_commandline(test_dirs, test)
